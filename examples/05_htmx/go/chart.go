// Chart data and rendering shared by the HTTP server (main.go) and the
// WASM demo (main_wasm.go).
package main
...
func parseHiddenList(csv string) []string {
	if csv == "" {
		return nil
	}
...
func toggleHidden(current []string, name string) []string {
	for i, h := range current {
		if h == name {
			return append(append([]string{}, current[:i]...), current[i+1:]...)
		}
	}
	return append(append([]string{}, current...), name)
}
...
// toggleButtonStyle is the inline style of a legend toggle button.
func toggleButtonStyle(hidden bool) string {
