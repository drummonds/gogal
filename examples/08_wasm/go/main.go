...
// Example 08: WASM Demo
//
// Full static chart running entirely in the browser via WebAssembly.
// Also serves the same chart on HTTP at http://localhost:1347.
package main
...
func buildChart() *gogal.Chart {
	...
	chart := gogal.NewLineChart(
		...
		gogal.WithTitle("Weather \u2014 15 June 2024"),
		gogal.WithGrid(true),
...
func main() {
	...
	fmt.Println("Serving WASM demo chart at http://localhost:1347")
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "text/html")
		fmt.Fprint(w, `<!DOCTYPE html>
<html><head><title>gogal - WASM Demo</title></head>
<body style="font-family: system-ui; padding: 2em; max-width: 900px; margin: 0 auto;">
<h1>WASM Demo (server-rendered)</h1>
`)
		chart.Render(w)
		fmt.Fprint(w, "\n</body></html>")
	})
	http.ListenAndServe(":1347", nil)
}
